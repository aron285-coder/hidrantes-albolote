...
- **Decisión:**
  1. **`hidrantes.fn_tareas_programadas()`**, `security definer`, dueño `hidrantes_migrador` (el que aplica las migraciones y es dueño de las tareas; `arranque-bd.sql` le da `select` en `cron.job` y `cron.job_run_details`). La misma consulta que `scripts/sql/tareas-programadas.sql`, sin la lista de esperadas: `falta` va siempre a `false` para que cada fila tenga la misma forma que las de la vigilancia y el panel anterior la pinte igual. En `plpgsql` para que la migración no falle en una base sin pg_cron. Sin `execute` para `anon` ni `authenticated`.
  2. **`fn_salud()`**, misma firma: `tareas` de `fn_tareas_programadas()` y `tareas_origen = 'en_vivo'`. Si falla con `insufficient_privilege`, `undefined_table`, `invalid_schema_name` o `undefined_function`, la foto de `config.tareas_programadas` con `tareas_origen = 'vigilancia'` y `tareas_medidas_en` (el `actualizado_en` de esa fila). Cualquier otro error sigue saliendo como error: no se esconde.
